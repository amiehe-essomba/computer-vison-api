...
def draw_boxes(image, boxes, box_classes, class_names, scores=None, path : str = ""):
    ...
    font =  None #= ImageFont.truetype( font="lat1", size=np.floor(3e-2 * image.size[1] + 0.5).astype('int32'))
        ...
        if path:
            try:
                with open(path, mode='a') as f:
                    f.write(f"label = {label}, boding_boxes = ({left}, {top}, {right}, {bottom})\n")
