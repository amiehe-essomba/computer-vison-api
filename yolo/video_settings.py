def slider_video(st, *details):
    ...
    if duration <= 4.0 :
        ...
        st.warning("Video too long > 4.0s, set video range to avoid streamlit cloud memory Error", icon="⚠️")

        col0, col1, col3 = st.columns(3)

        with col0:
            second = st.slider('time(s)', min_value=1, max_value=4, step=1, value=1)
        with col1:
